...
AS 
	...
	SELECT
		...
		ISNULL((
		SELECT
			TOP 1 ROUND(m2.VENTA_MEDIO_PAGO_COSTO, 0)
		FROM
			gd_esquema.maestra m2
		WHERE
			m2.VENTA_MEDIO_PAGO = m.VENTA_MEDIO_PAGO
	...
	FROM
		gd_esquema.maestra m
	WHERE
		m.VENTA_MEDIO_PAGO IS NOT NULL
	GROUP BY
		m.VENTA_MEDIO_PAGO
GO
...
AS
	...
	WHERE
		m.VENTA_CODIGO IS NOT NULL
		AND l.loca_localidad = m.CLIENTE_LOCALIDAD
		AND p.prov_provincia = m.CLIENTE_PROVINCIA
GO
